<template>
  <div class="detail">
    ...
    <div class="content" v-if="$store.state.detail._id">
      <!-- //data刚开始渲染拿不到 所以v-if -->
      <div class="header clear"><h2><img :src=" server.baseUrl+ $store.state.detail.detail.auth_icon" alt=""/></h2><p>{{$store.state.detail.detail.auth}}</p></div>
      <div class="cont">
        <h3>{{$store.state.detail.title}}/{{9|fillzero}}</h3>
        <div class="time"><p>{{$store.state.detail.time}}
          <span><img src="../assets/img/zan.png" alt=""/></span></p></div>
